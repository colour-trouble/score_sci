...
parser.add_argument('--testpath',
                    default="/home/yaping/projects/data/SCI/test_gray/")
...
cv2.imshow('ground truth', gt_batch[0,:,:,0].cpu().numpy()) # the first(1st) frame of the ground truth video
cv2.imshow('measurement', y_batch[0, :, :, 0].cpu().numpy())  # the first measurement (which compressing the 1st-8th frames of the video)
