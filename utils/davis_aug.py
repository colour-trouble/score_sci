...
class TrainData(Dataset):
    def __init__(self, train_data_dir, mask, in_channs=1):
        ...
        self.rgb2raw[1, :, :] = np.tile(g1, (mask_h // 2, mask_w // 2)) + np.tile(
            g2, (mask_h // 2, mask_w // 2)
        )
    ...
    def __getitem__(self, index):
        ...
        if self.color_flag is True:
            gt = np.zeros(
                [3, self.ratio, self.resize_h, self.resize_w], dtype=np.float32
            )
        ...
        for i, image_path in enumerate(self.img_files[index]):
            ...
            if not self.color_flag:
                ...
                pic_t = np.sum(
                    image[:, :, ::-1] * np.transpose(self.rgb2raw, (1, 2, 0)), axis=2
                )
                pic_t /= 255.0
                temp = image[:, :, ::-1]
