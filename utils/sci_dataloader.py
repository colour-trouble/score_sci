...
def load_mat(mask_location, key):
    if (
        get_matfile_version(_open_file(mask_location, appendmat=True)[0])[0] < 2
...
class SCITrainingDatasetSubset(Dataset):
    def __init__(self, gt_directory, meas_directory, mask_location):
        ...
        self.full_gt_filelist = [
            gt_directory + single_file for single_file in training_data
        ]
        self.full_meas_filelist = [
            meas_directory + single_file for single_file in training_data
        ]
    ...
    def __getitem__(self, item):
        ...
        data = {"gt": gt, "mask": self.mask, "meas": meas}
        return data
...
def load_test_data(matfile):
    # [1] load data
    if (
        get_matfile_version(_open_file(matfile, appendmat=True)[0])[0] < 2
    ):  # MATLAB .mat v7.2 or lower versions
        file = sio.loadmat(
            matfile
        )  # for '-v7.2' and lower version of .mat file (MATLAB)
    ...
    data = {"gt": orig / 255, "mask": mask, "meas": meas / 255}
    return data
